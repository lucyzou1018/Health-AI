...
def write_summary_report(
    ...
    lines = [
        ...
        f"- Avg Duration: {stats['avg']:.4f}s",
        f"- P95 Duration: {stats['p95']:.4f}s",
        f"- Min Duration: {dur_min:.4f}s",
        f"- Max Duration: {dur_max:.4f}s",
        f"- Std Deviation: {dur_std:.4f}s",
        "",
        "## Per-Run Durations",
    ]
    for i, d in enumerate(durations, 1):
        lines.append(f"- Run #{i}: {d:.4f}s")

    if stats["failures"]:
        lines.append("")
        lines.append("### Failure Samples")
        for item in stats["failures"][:5]:
            lines.append(f"- Run #{item.idx} exit {item.code}, duration {item.duration:.4f}s")
    lines.append("")
